...
async def generate_video(
    ...
    try:
        ...
        if not Path("SadTalker").exists():
            raise HTTPException(status_code=500, detail="SadTalker not installed. Run setup_real_sadtalker.py first")
        ...
        # Normalize arguments for direct generator (ensure defined and proper types)
        enhanceFace = True
        stabilization = True
...
async def process_video_generation_legacy(
    ...
    **kwargs
    ...
    try:
        # Step 1: Verify SadTalker installation (10%)
        update_progress(10, "Verifying SadTalker installation...")
        if not Path("SadTalker").exists():
            raise Exception("SadTalker not installed")
        ...
        from minimal_sadtalker import generate_video
        
        update_progress(60, "Generating video with AI...")
        result_path = generate_video(image_path, audio_path, output_path)
...
async def generate_fusion_video(
    ...
    preprocess: Optional[str] = Form("full")
):
    try:
        # Save uploads
        temp_dir = Path(config.get('paths.temp', 'temp'))
...
# Simple language/voice catalogs (alphabetical)
